//this script runs on the job details page and handles the job application form submission, including file upload and validation, and sends the data to a Netlify function that integrates with Bullhorn. It also manages UI feedback for success and error states.
...
document.addEventListener("DOMContentLoaded", () => {
  (function () {
    ...
    if (!form) {
      console.error(
        'Stopping script: <form id="job-application-form"> is required.',
      );
      return;
    }
    if (!submitBtn) {
    ...
    if (repEmailEl) {
      ...
      repEmailEl.href = representativeEmail
        ? `mailto:${representativeEmail}`
        : "";
    }
    ...
    function validateForm() {
      ...
      if (!uploadInput || uploadInput.files.length === 0) {
        ...
        if (!ALLOWED_EXTENSIONS.includes(ext)) {
          errors.push(
            `Alleen ${ALLOWED_EXTENSIONS.join(", ")} bestanden zijn toegestaan.`,
          );
        }
        if (file.size > MAX_FILE_SIZE) {
          errors.push(
            `Bestand is te groot (max ${MAX_FILE_SIZE / 1024 / 1024} MB).`,
          );
        }
      }
    ...
    async function postToBullhorn() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (DEBUG)
            console.error("Netlify function error:", response.status, text);
          return {
            success: false,
            message:
